 ...
 */
public class S01ChangeCdatChildListFragment extends Fragment implements FragmentWizard {
     ...
     */
    public static S01ChangeCdatChildListFragment newInstance(int index) {
        S01ChangeCdatChildListFragment fragment = new S01ChangeCdatChildListFragment();
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        adapter = new SelectChildArrayAdapter(context, childItems, ChildListType.CHANGE_CDAT);
    ...
    private void displayData() {
        ...
        fragmentContainer.setInstructions(listHeaderView, CURRENT_POSITION,
                R.string.desc_services_change_cdat_instruction_desc, false,
    ...
    private void setSelectedChildList() {
        ...
        ServiceChangeCdat changeCdat = app.getServiceChangeCdat();
        if (changeCdat != null) {
            changeCdat.setChildItems(selectedChildList);
            changeCdat.setChildIds(selectedChildIdList);
        }
        ...
        app.setServiceChangeCdat(new ServiceChangeCdat(selectedChildList, selectedChildIdList));
    }
    ...
    private void executeGetChildListAsyncTask() {
        ...
        ServicesGetChildListItemsTask asyncTask = new ServicesGetChildListItemsTask(
                context, ServiceAppType.CHANGE_CDAT,
                new MasterDataListener<ChildItem[]>() {
                    @Override
                    public void onMasterData(ChildItem[] items) {
                        if (items.length <= 0) {
                            ServerConnectionHelper.createConnectionIssueMessageBox(context, null);
                        } else {
